...
exports.convertFullClass = convertFullClass;
exports.convertComponent = convertComponent;
exports.convertProps = convertProps;
exports.convertPropSync = convertPropSync;
exports.convertModel = convertModel;
exports.convertEmits = convertEmits;
exports.convertEmitDecorator = convertEmitDecorator;
exports.convertComputed = convertComputed;
exports.convertWatch = convertWatch;
exports.convertMethod = convertMethod;
exports.convertLifecycle = convertLifecycle;
exports.convertRefs = convertRefs;
exports.convertVuex = convertVuex;
exports.convertDataFields = convertDataFields;
const LIFECYCLE_MAP = {
...
// ─── Internal helpers ─────────────────────────────────────────────────────────
function isComplexType(tsType, initValue) {
    if (initValue) {
        const v = initValue.trim();
        if (v.startsWith('{') || v.startsWith('[') || /^new\s/.test(v)) {
            return true;
        }
        if (/^(['"`]|-?\d|true|false|null|undefined)/.test(v)) {
            return false;
        }
    }
    if (tsType) {
        ...
        if (base.endsWith('[]') || /^Array\s*</.test(base)) {
            return true;
        }
        if (/^(Map|Set|WeakMap|WeakSet|Record|Promise|Observable)\s*</.test(base)) {
            return true;
        }
        ...
        if (parts.every(p => SIMPLE_TYPES.has(p) || /^['"`]/.test(p) || /^-?\d/.test(p))) {
            return false;
        }
    }
    return false;
}
function extractBody(text, openBrace) {
    let depth = 1;
    let i = openBrace + 1;
...
function detectVue3Imports(code) {
 ...
 */
function findComponentProp(body, key) {
    let i = 0;
    let depth = 0;
    while (i < body.length) {
        const c = body[i];
            ...
            if (m) {
                ...
                while (vEnd < body.length) {
                    ...
                    if (vc === '"' || vc === "'" || vc === '`') {
                        const q = vc;
                        vEnd++;
                        while (vEnd < body.length && body[vEnd] !== q) {
                            if (body[vEnd] === '\\')
                                vEnd++;
                            vEnd++;
                        }
...
function parseFiltersToFunctions(filtersBody) {
    const names = [];
    const decls = [];
    let i = 0;
    while (i < filtersBody.length) {
        while (i < filtersBody.length && /[\s,]/.test(filtersBody[i]))
            i++;
        if (i >= filtersBody.length)
            break;
        ...
        while (j < filtersBody.length && /[ \t]/.test(filtersBody[j]))
            j++;
        if (filtersBody[j] === '(') {
            ...
            while (k < filtersBody.length && pDepth > 0) {
                if (filtersBody[k] === '(')
                    pDepth++;
                else if (filtersBody[k] === ')')
                    pDepth--;
                k++;
            }
            const params = filtersBody.slice(j + 1, k - 1);
            while (k < filtersBody.length && /\s/.test(filtersBody[k]))
                k++;
            if (filtersBody[k] === '{') {
                const { body, end } = extractBody(filtersBody, k);
                names.push(name);
                decls.push(`function ${name}(${params}) {${body}}`);
                i = end;
                continue;
            }
        }
            ...
            if (fnMatch) {
                ...
                names.push(name);
                decls.push(`function ${name}(${fnMatch[1]}) {${body}}`);
                i = j + end;
                continue;
            }
            ...
            while (vEnd < rest.length) {
                ...
                if (vc === '"' || vc === "'" || vc === '`') {
                    const q = vc;
                    vEnd++;
                    while (vEnd < rest.length && rest[vEnd] !== q) {
                        if (rest[vEnd] === '\\')
                            vEnd++;
                        vEnd++;
                    }
            ...
            if (valueStr) {
                names.push(name);
                decls.push(`const ${name} = ${valueStr}`);
            }
            i = j + vEnd + (rest[vEnd] === ',' ? 1 : 0);
            continue;
        }
        i++;
    }
...
function parseMixinNames(value) {
...
function convertTemplateFilters(template) {
 ...
 */
function processComponentOptions(optBody) {
    let body = optBody;
    const extraDecls = [];
    let needsDefineAsyncComponent = false;
    // name, inheritAttrs → defineOptions
    const defineOptsEntries = [];
...
function convertScriptContent(scriptContent) {
    ...
    preamble = preamble.replace(/\bconst\s+(\w+)\s*=\s*\(\s*\)\s*=>\s*import\(([^)]+)\)/g, 'const $1 = defineAsyncComponent(() => import($2))');
 ...
 */
function convertFullClass(text) {
 ...
 */
function convertComponent(text) {
...
function convertProps(text) {
    const propRegex = /@Prop\(((?:[^()]*|\([^)]*\))*)\)\s+(?:(?:public|private|protected)\s+)?(?:readonly\s+)?(\w+)[!?]?\s*:\s*([^;\n]+);/g;
    const props = [];
    let match;
    ...
    const cleaned = text
        .replace(/@Prop\((?:[^()]*|\([^)]*\))*\)\s+(?:(?:public|private|protected)\s+)?(?:readonly\s+)?\w+[!?]?\s*:\s*[^;\n]+;\n?/g, '')
        .trim();
    return definePropsStr + '\n\n' + cleaned;
}
/** Converts @PropSync('prop') var!: T  →  const var = defineModel<T>('prop') */
function convertPropSync(text) {
    const pattern = /@PropSync\(\s*['"`](\w+)['"`](?:\s*,\s*(?:\{[^}]*\}))?\s*\)\s+(?:(?:public|private|protected)\s+)?(?:readonly\s+)?(\w+)[!?]?\s*:\s*([^;\n]+);/g;
    const decls = [];
...
function convertModel(text) {
    const pattern = /@Model\(\s*['"`][\w:]+['"`](?:\s*,\s*(?:\{[^}]*\}))?\s*\)\s+(?:(?:public|private|protected)\s+)?(?:readonly\s+)?(\w+)[!?]?\s*:\s*([^;\n]+);/g;
    const decls = [];
 ...
 */
function convertEmitDecorator(text) {
    const pattern = /@Emit\(\s*(?:['"`]([\w-]+)['"`])?\s*\)(?:\s*\/\/[^\n]*)?\s*(?:async\s+)?(\w+)\s*\(([^)]*)\)(?:\s*:\s*[^{]+)?\s*\{/g;
    const decls = [];
    const emitEvents = [];
    const segments = [];
    let match;
...
function convertWatch(text) {
    const pattern = /@Watch\(\s*['"`]([\w.]+)['"`](?:\s*,\s*(\{[^}]*\}))?\s*\)\s*(?:async\s+)?(\w+)\s*\(([^)]*)\)(?:\s*:\s*[^{]+)?\s*\{/g;
    const decls = [];
    const segments = [];
    let match;
    while ((match = pattern.exec(text)) !== null) {
        ...
        const options = [];
        if (optStr) {
            if (/immediate\s*:\s*true/.test(optStr)) {
                options.push('immediate: true');
            }
            if (/deep\s*:\s*true/.test(optStr)) {
                options.push('deep: true');
            }
        }
 ...
 */
function convertLifecycle(text) {
...
function convertRefs(text) {
    const pattern = /@Ref\(\s*(?:['"`](\w+)['"`])?\s*\)\s*(?:(?:public|private|protected)\s+)?(?:readonly\s+)?(\w+)[!?]?\s*:\s*([^;\n]+);/g;
    const decls = [];
...
function convertVuex(text) {
    const statePattern = /@State(?:\(\s*(?:['"`](\w+)['"`])?\s*\))?\s*(?:readonly\s+)?(\w+)[!?]?\s*:\s*[^;\n]+;/g;
    const getterPattern = /@Getter(?:\(\s*(?:['"`](\w+)['"`])?\s*\))?\s*(?:readonly\s+)?(\w+)[!?]?\s*:\s*[^;\n]+;/g;
    const actionPattern = /@Action(?:\(\s*(?:['"`](\w+)['"`])?\s*\))?\s*(?:readonly\s+)?(\w+)[!?]?\s*:\s*[^;\n]+;/g;
    const mutationPattern = /@Mutation(?:\(\s*(?:['"`](\w+)['"`])?\s*\))?\s*(?:readonly\s+)?(\w+)[!?]?\s*:\s*[^;\n]+;/g;
    const decls = [];
    let hasVuex = false;
    let result = text;
...
function convertDataFields(text) {
    const fieldStart = /^([ \t]*)(?:(public|private|protected)\s+)?(?:(readonly)\s+)?(\w+)[!?]?(?:\s*:\s*([^=;\n]+?))?\s*=\s*/gm;
    const decls = [];
    const segments = [];
    let match;
    while ((match = fieldStart.exec(text)) !== null) {
        const [fullStart, indent, accessMod, readonlyMod, name, tsType] = match;
        if (!accessMod && !readonlyMod && !tsType) {
            continue;
        }
