...
function activate(context) {
    ...
    migrationCommands.forEach(({ id, fn }) => {
        context.subscriptions.push(vscode.commands.registerCommand(id, async () => {
            ...
            if (doc.languageId !== 'vue') {
                vscode.window.showErrorMessage('Vue Migration Helper only works in .vue files');
                return;
            }
            ...
            try {
                ...
                vscode.window.setStatusBarMessage(`Successfully applied: ${id.split('.').pop()}`, 3000);
            }
            catch (error) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vue-migrate.showMenu', async () => {
    ...
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider('vue', new VueMigratorProvider(migrationCommands), {
    ...
    provideCodeActions(document, range) {
        ...
        this.commands.forEach((cmd) => {
            if (cmd.pattern && lineText.includes(cmd.pattern)) {
                const action = new vscode.CodeAction(`✨ Migrate ${cmd.label}`, vscode.CodeActionKind.RefactorRewrite);
                action.command = {
                    command: cmd.id,
                    title: cmd.label,
                };
